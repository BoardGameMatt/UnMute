/**
 * Mint a SwitchCode lobby on Pack A and print host/join URLs.
 * npx tsx scripts/mint-code-switch-session.ts
 *
 * Requires migration 025 (code_switch_* tables + Pack A).
 */
...
async function main(): Promise<void> {
  ...
  const { data: protocol, error: pErr } = await supabase
    ...
    .single();
  if (pErr || !protocol) {
    console.error("code-switch protocol missing:", pErr?.message);
    process.exit(1);
  }
  ...
  const { data: pack, error: packErr } = await supabase
    ...
    .maybeSingle();
  if (packErr || !pack) {
    console.error("SwitchCode Pack A missing:", packErr?.message);
    process.exit(1);
  }
  ...
  console.log("SwitchCode lobby (Pack A).");
