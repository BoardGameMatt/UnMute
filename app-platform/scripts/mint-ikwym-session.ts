/**
 * Mint an I Know What You Meme lobby on Pack A and print host/join URLs.
 * npx tsx scripts/mint-ikwym-session.ts
 *
 ...
 * Requires migration 023 (ikwym_* tables + Pack A).
 */
...
async function main(): Promise<void> {
  ...
  const { data: protocol, error: pErr } = await supabase
    .from("protocols")
    .select("id")
    .eq("slug", "i-know-what-you-meme")
    .single();
  if (pErr || !protocol) {
    console.error("i-know-what-you-meme protocol missing:", pErr?.message);
    process.exit(1);
  }
  ...
  const { data: pack, error: packErr } = await supabase
    ...
    .maybeSingle();
  if (packErr || !pack) {
    console.error("I Know What You Meme Pack A missing:", packErr?.message);
    process.exit(1);
  }
  ...
  if (demoTeam) {
    ...
    const { data: team, error: tErr } = await supabase
      ...
      .upsert(
        {
          name: "I Know What You Meme Playtest",
          join_code: "IKWYMT",
          require_auth: false,
        },
  ...
  console.log("I Know What You Meme lobby (Pack A).");
  ...
  console.log("Needs NEXT_PUBLIC_GIPHY_API_KEY in .env.local.");
}
