/**
 * Mint a Rank and File lobby on Pack A and print host/join URLs.
 * npx tsx scripts/mint-rank-and-file-session.ts
 *
 * Requires migration 027 (rank_and_file_* tables + Pack A).
 */
...
async function main(): Promise<void> {
  ...
  const { data: protocol, error: pErr } = await supabase
    .from("protocols")
    .select("id")
    .eq("slug", "rank-and-file")
    .single();
  if (pErr || !protocol) {
    console.error("rank-and-file protocol missing:", pErr?.message);
    process.exit(1);
  }
  ...
  const { data: pack, error: packErr } = await supabase
    ...
    .maybeSingle();
  if (packErr || !pack) {
    console.error("Rank and File Pack A missing:", packErr?.message);
    process.exit(1);
  }
  ...
  console.log("Rank and File lobby (Pack A).");
  ...
  console.log("");
  console.log("Use localhost (dev server).");
}
