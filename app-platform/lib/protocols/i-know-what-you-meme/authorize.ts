/**
 * Talk Track-style authorization. Service client is created only after cookie + membership.
 */
...
export type IkwymAuthFailure = {
  ok: false;
  status: 401 | 403 | 404 | 500;
  error: string;
};

export type IkwymMemberAuth = {
  ok: true;
...
export async function authorizeIkwymParticipant(
  sessionId: string,
  options?: { allowCompleted?: boolean }
): Promise<IkwymMemberAuth | IkwymAuthFailure> {
  ...
  if (protocolSlug !== "i-know-what-you-meme") {
    return { ok: false, status: 404, error: "I Know What You Meme is not active on this session." };
  }
