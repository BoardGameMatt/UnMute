/**
 * Rank and File authorization. Service client is created only after cookie + membership.
 */
...
export type RankAndFileAuthFailure = {
  ok: false;
  status: 401 | 403 | 404 | 500;
  error: string;
};

export type RankAndFileMemberAuth = {
  ok: true;
...
export async function authorizeRankAndFileParticipant(
  sessionId: string
): Promise<RankAndFileMemberAuth | RankAndFileAuthFailure> {
  ...
  if (protocolSlug !== "rank-and-file") {
    return { ok: false, status: 404, error: "Rank and File is not active on this session." };
  }
